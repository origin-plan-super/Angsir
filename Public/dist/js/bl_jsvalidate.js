...
var formSubmitCheck=function(args){
	...
	if(!allSuccess){	
		...
		if($(".resume-site-nav").length>0){
			firstFalseEleTop=$(firstFalseEle).offset().top-70;			
		}
		if($(".pagewrap").length>0){
			var mheaderH=$(".header").height();
			firstFalseEleTop=$(firstFalseEle).offset().top-mheaderH;
		}
		...
		if (args.minlength != undefined) {
			if(vallength<args.minlength[0]){
				if(args.minlength.length==2){
					var curmsg=args.minlength[1]+args.minlength[0];
					setErrormsg({"parentEle":args.parentEle,"msg":curmsg});
        ...
        $.ajax({
            ...
            success: function(data){
            	...
            	if(args_success!=undefined){
            		...
            		if(callbackAgrs.msg!=undefined){	
            			setErrormsg({"parentEle":args.parentEle,"msg":callbackAgrs.msg});
            			checkState=true;  
            		}else{
            			checkState=false;             			 
            		}
            	...
            	if(checkState==true){            		
        			...
        			if($(args.curOjb).attr("ajaxCheck")=="submit"){        				
        				if(callbackAgrs.callbackFun!=undefined){
        					callbackAgrs.callbackFun.call(null);        									
        				}else{
